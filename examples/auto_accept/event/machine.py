...
def main(host, port, cert, username, password):
    ...
    try:
        ...
        error(
            f"Could not connect to Galileo at {host} on port {port} with cert {cert}.",
            1,
        )
    ...
    def on_p2l_request(launch_pad_id):
        try:
            ...
            error(
                f"Something went wrong while accepting request from '{launch_pad_id}'."
            )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Commands a running Galileo daemon to automatically accept all P2L "
        "requests. Requires that galileod and galileo-cli are running."
    )
    parser.add_argument(
        "--host",
        default="https://localhost",
        help="The IPv4 address of the daemon",
        type=str,
    )
    parser.add_argument("--port", default=5000, help="The port of the daemon", type=int)
    parser.add_argument(
        "--cert",
        default="galileod.crt",
        help="The SSL certificate for the daemon",
        type=str,
    )
    parser.add_argument("--username", default="", help="Username for login", type=str)
    parser.add_argument(
        "--password", default="", help="Password for username", type=str
    )
